...
async function initDB() {
  ...
  await db.query(`ALTER TABLE apps ADD COLUMN IF NOT EXISTS proxy_enabled BOOLEAN NOT NULL DEFAULT FALSE`);
  await db.query(`ALTER TABLE apps ADD COLUMN IF NOT EXISTS proxy_url TEXT NOT NULL DEFAULT ''`);
  await db.query(`ALTER TABLE apps ADD COLUMN IF NOT EXISTS proxy_host TEXT NOT NULL DEFAULT ''`);
  await db.query(`ALTER TABLE apps ADD COLUMN IF NOT EXISTS proxy_port INTEGER NOT NULL DEFAULT 8080`);
  await db.query(`ALTER TABLE apps ADD COLUMN IF NOT EXISTS proxy_username TEXT NOT NULL DEFAULT ''`);
  await db.query(`ALTER TABLE apps ADD COLUMN IF NOT EXISTS proxy_password TEXT NOT NULL DEFAULT ''`);
  await db.query(`ALTER TABLE apps ADD COLUMN IF NOT EXISTS ipv6_fraud_message TEXT NOT NULL DEFAULT ''`);
  await db.query(`ALTER TABLE apps ADD COLUMN IF NOT EXISTS device_fraud_message TEXT NOT NULL DEFAULT ''`);
...
app.get('/api/admin/apps/:id/proxy', requireSuperAuth, async (req, res) => {
  const { rows } = await db.query(
    'SELECT proxy_enabled, proxy_url, proxy_host, proxy_port, proxy_username, proxy_password, ipv6_fraud_message, device_fraud_message FROM apps WHERE id = $1',
    [req.params.id]
  );
  if (!rows.length) return res.status(404).json({ error: 'App not found' });
  res.json(rows[0]);
...
app.put('/api/admin/apps/:id/proxy', requireSuperAuth, async (req, res) => {
  const { proxy_enabled, proxy_url, proxy_host, proxy_port, proxy_username, proxy_password, ipv6_fraud_message, device_fraud_message } = req.body;
  if (typeof proxy_enabled !== 'boolean') return res.status(400).json({ error: 'proxy_enabled must be boolean' });
  const port = parseInt(proxy_port) || 8080;
  const { rows } = await db.query(
    `UPDATE apps SET proxy_enabled=$1, proxy_url=$2, proxy_host=$3, proxy_port=$4, proxy_username=$5, proxy_password=$6,
     ipv6_fraud_message=$7, device_fraud_message=$8
     WHERE id=$9 RETURNING proxy_enabled, proxy_url, proxy_host, proxy_port, proxy_username, proxy_password, ipv6_fraud_message, device_fraud_message`,
    [proxy_enabled, (proxy_url||'').trim(), (proxy_host||'').trim(), port,
     (proxy_username||'').trim(), (proxy_password||'').trim(),
     (ipv6_fraud_message||'').trim(), (device_fraud_message||'').trim(), req.params.id]
  );
  if (!rows.length) return res.status(404).json({ error: 'App not found' });
  res.json(rows[0]);
});

// ── Proxy Settings (app admin) ────────────────────────────────────────────────
app.get('/api/panel/:apiKey/proxy', requireAppAuth, async (req, res) => {
  const { rows } = await db.query(
    'SELECT proxy_enabled, proxy_url, proxy_host, proxy_port, proxy_username, proxy_password, ipv6_fraud_message, device_fraud_message FROM apps WHERE id = $1',
    [req.appId]
  );
...
app.put('/api/panel/:apiKey/proxy', requireAppAuth, async (req, res) => {
  const { proxy_enabled, proxy_url, proxy_host, proxy_port, proxy_username, proxy_password, ipv6_fraud_message, device_fraud_message } = req.body;
  if (typeof proxy_enabled !== 'boolean') return res.status(400).json({ error: 'proxy_enabled must be boolean' });
  const port = parseInt(proxy_port) || 8080;
  const { rows } = await db.query(
    ...
    [proxy_enabled, (proxy_url||'').trim(), (proxy_host||'').trim(), port,
     (proxy_username||'').trim(), (proxy_password||'').trim(),
     (ipv6_fraud_message||'').trim(), (device_fraud_message||'').trim(), req.appId]
  );
  if (!rows.length) return res.status(404).json({ error: 'App not found' });
  res.json(rows[0]);
});

// ── Public Proxy Config endpoint (no auth — for startup scripts / apps) ───────
app.get('/api/proxy-config/:apiKey', async (req, res) => {
  try {
    const { rows } = await db.query(
      'SELECT proxy_enabled, proxy_url, proxy_host, proxy_port, proxy_username, proxy_password FROM apps WHERE api_key = $1',
      [req.params.apiKey]
    );
    if (!rows.length) return res.status(404).json({ error: 'App not found' });
    const p = rows[0];
...
app.get('/api/dialog/config/:apiKey', async (req, res) => {
  try {
    const { rows: appRows } = await db.query('SELECT id, proxy_enabled, proxy_url, proxy_host, proxy_port, proxy_username, proxy_password, ipv6_fraud_message, device_fraud_message FROM apps WHERE api_key = $1', [req.params.apiKey]);
    if (!appRows.length) return res.status(404).json({ error: 'App not found' });
    const appId = appRows[0].id;
    ...
    if (ipv6Final && ipv6TrackingOn) {
      ...
      if (v6Rows.length) {
        ipv6Fraud = true;
        const _ipv6Msg = appRows[0].ipv6_fraud_message || 'প্রতারণামূলক অ্যাকাউন্ট খোলা হয়েছে দয়া করে আইপি চেঞ্জ করুন';
        config.ipv6_fraud = true;
        config.ipv6_fraud_message = _ipv6Msg;
        if (!config.fraud_detected) {
          config.fraud_detected = true;
          config.fraud_message = _ipv6Msg;
        }
        await db.query(
      ...
      if (v6Rows.length) {
        ipv6Fraud = true;
        const _ipv6Msg2 = appRows[0].ipv6_fraud_message || 'প্রতারণামূলক অ্যাকাউন্ট খোলা হয়েছে দয়া করে আইপি চেঞ্জ করুন';
        config.ipv6_fraud = true;
        config.ipv6_fraud_message = _ipv6Msg2;
        if (!config.fraud_detected) {
          config.fraud_detected = true;
          config.fraud_message = _ipv6Msg2;
        }
        await db.query(
          'UPDATE ipv6_records SET hit_count = hit_count + 1, last_seen = NOW() WHERE app_id = $1 AND ipv6 = $2',
          [appId, ipv6Final]
    ...
    if (deviceId && devTrackingOn) {
      ...
      if (devRows.length) {
        ...
        const _devMsg = appRows[0].device_fraud_message || 'একই ডিভাইস পাওয়া গিয়েছে দয়া করে এইটাও চেন্জ করুন';
        config.device_duplicate = true;
        config.device_duplicate_message = _devMsg;
        if (!config.fraud_detected) {
          config.fraud_detected = true;
          config.fraud_message = _devMsg;
        }
        await db.query(
      ...
      if (devRows.length) {
        deviceDuplicate = true;
        const _devMsg2 = appRows[0].device_fraud_message || 'একই ডিভাইস পাওয়া গিয়েছে দয়া করে এইটাও চেন্জ করুন';
        config.device_duplicate = true;
        config.device_duplicate_message = _devMsg2;
        if (!config.fraud_detected) {
          config.fraud_detected = true;
          config.fraud_message = _devMsg2;
        }
        await db.query(
          'UPDATE device_id_records SET hit_count = hit_count + 1, last_seen = NOW() WHERE app_id = $1 AND device_id = $2',
          [appId, deviceId]
    ...
    const p = appRows[0];
    config.proxy_enabled  = p.proxy_enabled || false;
    config.proxy_url      = p.proxy_url      || '';
    config.proxy_host     = p.proxy_host     || '';
    config.proxy_port     = p.proxy_port     || 8080;
    config.proxy_username = p.proxy_username || '';
    config.proxy_password = p.proxy_password || '';
